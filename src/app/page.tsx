...
function CaptureCard() {
  return <div className="capture-card"><span className="step-chip">Step 02 — Identify</span><div className="capture-rate"><div className="rate-num"><strong>35%</strong><span>of US visitors identified<br />on high-intent traffic</span></div><div className="rate-bar" role="img" aria-label="35 percent of visitors identified"><i /></div></div><p className="capture-sub">We match high-intent visits to real people and enrich every profile:</p><ul className="capture-fields">{captureFields.map(field => <li key={field.label}><Check /><span>{field.label}</span><b>{field.value}</b></li>)}</ul></div>
}
